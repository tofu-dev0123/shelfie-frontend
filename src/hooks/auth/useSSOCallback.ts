...
/**
 * SSOコールバックページのロジックを管理するフック。
 * OAuthプロバイダーからのリダイレクト後にClerkのセッションを確立し、適切なページへ遷移する。
 * @returns なし
 */
export const useSSOCallback = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ケース1: サインイン完了（ログインフロー）
      if (signIn.status === "complete") {
        await signIn.finalize({ navigate: toSignupContinue });
        return;
      }

      // ケース2: signUpフロー → 既存Clerkアカウントあり（signInに転送してfinalize）
      ...
      // ケース3: signInフロー → 新規ユーザー（signUpに転送）
      ...
      // ケース5: 追加要件あり（Clerkがusernameを必須とする場合など）
